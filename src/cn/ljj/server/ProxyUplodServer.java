...
public class ProxyUplodServer extends HttpServlet implements StaticDefines {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		conn.setConnectTimeout(5 * 1000);
		InputStream inStream = conn.getInputStream();
		File tempFile = new File(UPLOAD_SAVE_FILE_ROOT_PATH, ".proxyUploadTemp/" + System.currentTimeMillis() + "_" + Thread.currentThread().getId());
		...
		tempFile.delete();
		response.getWriter().write("{url=\"" + qCloudUrl + "\"}");
	}
